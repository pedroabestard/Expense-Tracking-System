...
def test_fetch_expenses_for_date_aug15():
    ...
    # since these tests run in production, the results can variate, so we should write
    # these test calling a test database (instead of db_helper). Since this project is
    # entry level, we will call the same database
